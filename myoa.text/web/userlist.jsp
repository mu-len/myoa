...
<head>
	...
	<script type="text/javascript">
		function permissionFormSubmit() {
			var boo=true;
			if($("#name").val()==""&&boo){
				alert("账号不能为空")
				boo=false;
			}else if($("#password").val()==""&&boo){
				alert("密码不能为空")
				boo=false;
			}else if($("#email").val()==""&&boo){
				alert("邮箱不能为空")
				boo=false;
			}
			if(boo){
    ...
    <form class="form-inline">
        <div class="panel panel-default">
            <div class="panel-heading">用户列表&nbsp;&nbsp;&nbsp;
               <button type="button" class="btn btn-primary" title="新建" data-toggle="modal" data-target="#createUserModal" onclick="findRolesName()">新建用户</button></div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<table class="table table-bordered table-striped" width="800px">
						<tr>
							<td>帐号</td>
							<td><input class="form-control" name="name" placeholder="名称" id="name"></td>
						</tr>					
						<tr>
							<td>初始密码</td>
							<td><input class="form-control" type="password" name="password" placeholder="名称" id="password"></td>
						</tr>
						<tr>
						    <td>电子邮箱</td>
						    <td><input class="form-control" name="email" placeholder="链接" id="email">
						    </td>
						</tr>
						<tr>
						    <td>级别</td>
						    <td>
						    	<select class="form-control" name="role" onchange="getNextManager(this.value)" id="selectRoles">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-success" data-dismiss="modal"
						aria-hidden="true" onclick="permissionFormSubmit()">保存</button>
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog" >
	...
	<script type="text/javascript">
		...
		function findRolesName() {
			var url ="findRolesName";
			$.getJSON(url,function (rolesName) {
				$("#selectRoles").empty()
				$.each(rolesName,function (i,roles) {
					var opt=$("<option value='"+roles.id+"'>"+roles.name+"</option>")

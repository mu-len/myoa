...
<head>
	...
	<script type="text/javascript">
		function beChecks(id,boo) {
			var checkChildren = document.getElementById(id).parentElement.nextElementSibling.children
			for(var i=0;i<checkChildren.length;i++){
				checkChildren[i].checked=boo;
			}
		}
				                ...
				                <c:forEach var="role" items="${allRoles}">
				                	<tr id="${role.id}">
				                		...
				                		<td>
											...
											<a href="delSysRole?id=${role.id}" class="btn btn-danger btn-xs"
											onclick="if(confirm('确定要删除该角色吗？')==false)return false">
                		...
                		<tr>
                			<td>
                			 <input id="chk${menu.id}" type="checkbox" name="permissionIds" value="${menu.id}"
									onclick="beChecks(this.id,this.checked)"/>
                			 ${menu.name}
                			</td>
                			<td>
                				<c:forEach var="permission" items="${menu.children}">
                				    <input id="chk${permission.id}" type="checkbox" name="permissionIds" value="${permission.id}" onclick="brother(this.id)"/>

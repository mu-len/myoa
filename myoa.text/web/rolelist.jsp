...
<head>
	...
	<script type="text/javascript">
		function beChecks(id,boo) {
			var checkers=document.getElementById(id).parentElement.nextElementSibling.children;
			for(var i=0;i<checkers.length;i++){
				checkers[i].checked=boo;
			}
		}

		function brother(id) {
			var brother=document.getElementById(id).parentElement.children;
			var parent=document.getElementById(id).parentElement.previousElementSibling.children[0];
...
<div class="page-content">
                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
						<c:forEach var="menu" items="${allPermissions}">
		                    <tr>
		                        <td>
		                        <input type="checkbox" name="permissionIds" value="${menu.id}" id="${menu.id}" onclick="beChecks(this.id,this.checked)"/>
		                        ${menu.name}</td>
		                        <td>
		                        	<c:forEach var="subPermission" items="${menu.children}">
		                        		<input type="checkbox" name="permissionIds" value="${subPermission.id}" onclick="brother(this.id)" id="${subPermission.id}"/>
	...
	<div class="modal fade" id="editModal" tabindex="-1" role="dialog"
	  ...
	  <form id="Form" action="saveSubmitPermission" method="post">
